...
def to_mod_list(employee_list):
   """ Modifies the employee list of dictionaries into list of employee-department 
   strings
   [IMPLEMENT ME] 
...
def generate_usernames(mod_list):
   ...
   Returns:
      list - A list of usernames consisting of name + department delimited
        by underscores.
   """
   ### WRITE SOLUTION CODE HERE
   print(mod_list)
   L1 = [x.replace(' ', '_') for x in mod_list]
...
def map_id_to_initial(employee_list):
   ...
   [IMPLEMENT ME] 
      1. Use dictionary comprehension to map each employee's id (value) 
      to the first letter in their name (key)
   ...
   Returns:
      dict - A dictionary mapping an employee's id (value) 
      to their first initial (key).
   """
   ### WRITE SOLUTION CODE HERE
   id_initial_map = {employee['name'][0]: employee['id'] for employee in employee_list}
